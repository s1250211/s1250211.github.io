<template>
 <body>
   <i class="time">TIME</i>
   <i class="is">IS</i>
   <i class="money">MONEY.</i>
   <header>
        <i>Ryo Otake</i>
        <ul>
          <li>
            <a href="https://www.facebook.com/626RyOtK">
              <fa-icon class="color f" :icon="['fab', 'facebook-f']" size="1x"/>
              <i class="fa fa-facebook " aria-hidden="true"></i>
            </a>
          </li>
          <!-- <li>
            <a href="#">
              <fa-icon class="color t" :icon="['fab', 'twitter']" size="1x"/>
              <i class="fab fa-twitter "></i>
            </a>
          </li>
          <li>
            <a href="https://wear.jp/ryo626aizu/">
              <fa-icon class="color s" :icon="['fas', 'tshirt']" size="1x"/>
              <i class="fa fa-tshirt" aria-hidden="true"></i>
            </a>
          </li> -->
          <li>
              <a href="https://www.instagram.com/the.other.ryo">
              <fa-icon class="color i" :icon="['fab', 'instagram']" size="1x"/>
              <i class="fa fa-instagram" aria-hidden="true"></i>
              </a> 
          </li>
        </ul>
   </header>
    
    <div class="box">
      <h1>
        <p>Welcome to my portfolio !</P>
        <p>Please scroll down↓↓↓</p>
      </h1>
      <img src="./Active Chameleon.jpg">
    </div>

 <div class="main">
   <span class=A>◉</span>
   <span class=B>◉</span>
   <span class=C>◉</span>
   <span class=D>◉</span>
   <span class=E>◉</span>
   <span class=F>◉</span>
   <span class=G>◉</span>
   
  <img src="./りょう.JPG">
   <div class="life">
     <div class="content">
       <br><br>
       <ul>
         <li><h4>大学4年生　会津大学コンピューター理工学部</h4></li>
         <li><h4>大武　凌(22)</h4></li>
         <li>栃木・宇都宮市出身</li><br>
         <li>'05 栃木県宇都宮市立陽西保育園　卒園</li>
         <li>'11 栃木県宇都宮市立桜小学校　卒業</li>
         <li>'14 栃木県宇都宮市立陽西中学校　卒業</li>
         <li>'17 栃木県立宇都宮北高校　卒業</li>
         <li>'17 福島県立会津大学　入学</li>
         <li>'18 牛角会津若松店にアルバイトとして入社</li>
         <li>恋人いない歴 2年と7ヶ月</li>
       </ul>
     </div>
   </div>
   <div class="profile">
     <div class="content">
       <ul>
         <li><h4>-趣味-</h4></li>
         <li>動画観賞、ドライブ、筋トレ、スケボー</li>
         <li><h4>-特技-</h4></li>
         <li>サッカー</li>
         <li><h4>-好きなお酒の銘柄-</h4></li>
         <li>飛露喜(日本酒)</li>
         <li><h4>-好きな異性の髪型-</h4></li>
         <li>ボブの外ハネ or ポニーテール</li>
         <li><h4>-好きなアーティスト-</h4></li>
         <li>YOASOBI、BLACKPINK</li>
         <li><h4>-所属しているサークル-</h4></li>
         <li>バレーボールサークル(部長をやってました)</li>
         <li><h4>-今年必ずしたいこと-</h4></li>
         <li>断捨離、ミラーレス一眼カメラの購入</li>
       </ul>
     </div>
   </div>
   <div class="past">
     <div class="content">
     </div>
   </div>
   <div class="now">
     <div class="content">
     <ul>
       <h2>-About the current me-</h2>
       <br>
       <li>現在、就職活動とアルバイトと研究を行っています。</li>
       <li>"自分のやりたい事ができるか" と "社風が合うかどうか" を軸に就職活動を行っています。</li>
       <li>アルバイトは、会津の牛角でしており、バイトリーダーを任されています。最近はお客さんの入りも戻ってきており、大変有難いです。</li>
       <li>研究のテーマは、指向性スピーカーを用いた車内音響の改善で、10月の中間発表に向けて少しずつ作業を進めています。</li>
     </ul>

     </div>
   </div>
   <div class="future">
     <div class="content">
     <ul>
       <h2>-From now on-</h2>
       <br>
       <li>メインでやってくことは就活です。自分と合う会社さんと巡り合いたいと思っています。
         就職活動と同時進行でアルバイトと研究を少しずつ行っていきます。
         就職活動が終わり次第、研究と入社へ向けて知識や経験を積みながら、友人との思い出作りをしたいと思っています。
         その他に、英語力をつけたり、気になることをしてみたり、楽しい・面白いと思うことはたくさん着手していこうと思います。
         私は欲張りなので、多くのことを経験してガンガン成長したいと思っています。<br><br>
         <h3>ハイスペック人間を目指して、頑張っていきます！！！</h3>
        </li>
       <li></li>
       <li></li>
       <li></li>
     </ul>
     </div>
   </div>
   <div class="movie">
     <a href="https://youtu.be/GYolJzlfeJ4"><button><p>動画作品</p></button></a>
   </div>
   <div class="photo">
   </div>
 </div>

 <footer>
  <div class=content>
<p>s1250211@gmail.com </p>
<p>Last edit date：2020/07/13 </p>
  </div>
  </footer>

<!--<div class="bottom">
  <ul>
    <li>
      <a href="#">
        <fa-icon class=color :icon="['fab', 'facebook-f']" size="3x"/>
        <i class="fa fa-facebook" aria-hidden="true"></i>
        <span> -Facebook-</span>
      </a>
    </li>
    <li>
      <a href="#">
        <fa-icon class=color :icon="['fab', 'twitter']" size="3x"/>
        <i class="fa fa-twitter" aria-hidden="true"></i>
        <span> -Twitter-</span>
      </a>
    </li>
    <li>
        <a href="#">
        <fa-icon class=color :icon="['fab', 'github']" size="3x"/>
        <i class="fa fa-google-plus" aria-hidden="true"></i>
        <span> -Github-</span>
      </a>
    </li>
    <li>
      <a href="https://www.instagram.com/ryo_626_take">
        <fa-icon class=color :icon="['fab', 'instagram']" size="3x"/>
        <i class="fa fa-instagram" aria-hidden="true"></i>
        <span> -Instagram-</span>
      </a>
    </li>
  </ul>
  </div>
  -->
 </body>
</template>


 <style>
body {
  height:1300px;
  max-width: 1500px;
  margin: auto;
  padding:0;
  font-family: 'Roboto Condensed', sans-serif;
  background-repeat: no-repeat;
  background-attachment: fixed;
  background-size: cover;
  background-color:#1c6e74;
}
body i{
  position:fixed;
  font-family:serif;
  font-size:190px;
  color:#fff;
  z-index: 0;
}
.time {
top:100px;
opacity: 0.5;
}
.is{
top:240px;
left:400px;
opacity: 0.5;
}
.money{
top:380px;
left:490px;
opacity: 0.5;
}
header{
  background-color:rgba(1, 20, 187,0.8);
  height:75px;
  width:100%;
  position:fixed;
  top:0px;
  z-index: 10;
}

header i{
  font-size:45px;
  font-family: cursive;
  color:rgba(159, 255, 223);
  margin: 0px auto;
  left:70px;
  
}
header ul{
  float:right;
  list-style:none;
}
header ul li{
  display:inline-block;
  font-size: 50px;
  padding:0px 10px;
  color:#fff;
  width:80px;
  text-align: center;
  transition: 0.5s;
}
header ul li a{
  color:#fff;
  transition: 0.5s;
}
header ul li:hover{
background: #fff;
transition: 0.5s;
}
header ul li:hover .f{
  color:rgb(57, 113, 218);
  animation: anime1 0.5s ease 0s infinite alternate;
  transition: 0.5s;
}
header ul li:hover .t{
  color:deepskyblue;
  animation: anime1 0.5s ease 0s infinite alternate;
  transition: 0.5s;
}
header ul li:hover .s{
  color:#a1a0a0;
  animation: anime1 0.5s ease 0s infinite alternate;
  transition: 0.5s;
}
header ul li:hover .i{
  color:rgb(255, 11, 72);
  animation: anime1 0.5s ease 0s infinite alternate;
  transition: 0.5s;
}
@keyframes anime1 {
  from {
    transform: scale(1,1);
  }
  to {
    transform: scale(0.8,0.8);
  }
}

.box{
  height:700px
}

.box img {
  position:absolute;
  top:350px;
  left:100px;
  height:280px;
  width:280px;
  border-radius: 100px;
  animation: anime2 1s ease 0s infinite alternate;
}
@keyframes anime2 {
  from {
    transform: rotate(7deg);
  }
  to {
    transform: rotate(-7deg);
  }
}
.box h1 {
  position:absolute;
  color:#262626;
  font-size: 50px;
  display: block;
  margin-top:200px;
  margin-left:410px;
  background-color:#fff;
  padding:70px 60px;
  border-radius:100px 100px 100px 0px;
  text-align: center;
}
.main{
  height:1200px;
  background-image: url(https://free-texture.net/wp-content/uploads/CorkBoard01.jpg);
  border:inset 20px #6b3319;
  align-items: center;
  background-repeat: repeat;
  margin:50px 50px;
  position:relative;
  z-index: 1;
}
.main img{
  object-fit: cover;
  position: absolute;
  height:300px;
  width:300px;
  margin-top:50px;
  margin-left:30px;
  transform: rotate(-5deg);
  transition: 0.5s;
}
.main img:hover{
transform: scale(1.1,1.1);
z-index:4;
transition: 0.5s;
}
.main span{
  color:rgb(221, 162, 0);
  font-size:30px;
  display:inline-block;
  position:absolute;
  z-index: 3;
}
.A{
  top:60px;
  left:145px;
}
.B{
  top:58px;
  left:520px;
}
.C{
  top:80px;
  left:900px;
}
.D{
  top:510px;
  left:55px;
}
.E{
  top:500px;
  left:490px;
}
.F{
  top:600px;
  left:595px;
}
.G{
  top:635px;
  left:1070px;
}
.life{
  position: absolute;
  background-color:rgb(236, 255, 227);
  height:450px;
  width:400px;
  top:50px;
  left:330px;
  transform: rotate(3deg);
  transition: 0.5s;
}
.life:hover{
transform: scale(1.1,1.1);
z-index:4;
transition: 0.5s;
}
.life ul{
  list-style: none;
  padding-left:40px;
  padding-right:20px;
  padding-top:10px;
  line-height: 30px;
  font-family:monospace;
}

.profile{
  position:absolute;
  background-color:rgb(255, 226, 254);
  height:530px;
  width:380px;
  top:75px;
  left:730px; 
  transition: 0.5s;
}
.profile:hover{
transform: scale(1.1,1.1);
z-index:4;
transition: 0.5s;
}
.profile ul{
  list-style: none;
  padding-left:20px;
  padding-right:20px;
  padding-top:55px;
  line-height: 30px;
  font-family:monospace;
}
.now{
  position:absolute;
  background-color:rgb(255, 253, 224);
  height:385px;
  width:500px;
  top:500px;
  margin-left:40px;
  transform: rotate(-1deg);
  transition: 0.5s;
}
.now:hover{
transform: scale(1.1,1.1);
z-index:4;
transition: 0.5s;
}
.now ul{
  list-style:square;
  padding-left:40px;
  padding-right:20px;
  padding-top:40px;
  line-height: 27px;
  font-family:monospace;
}

.future{
  position:absolute;
  background-color:rgb(255, 225, 225);
  height:450px;
  width:550px;
  top:610px;
  left:560px;
  transform: rotate(4deg);
  transition: 0.5s;
}
.future:hover{
transform: scale(1.1,1.1);
z-index:4;
transition: 0.5s;
}
.future ul{
  list-style: none;
  padding-left:40px;
  padding-right:20px;
  padding-top:50px;
  line-height: 30px;
  font-family:monospace;
}
.photo{
  height:240px;
  width:400px;
  background-image: url(./すいぞっかん.jpg);
  top:885px;
  left:-263px;
  transform: rotate(-5deg);
  border:solid 7px #e0e0e0;
  background-size: 100% 100%;
  transition: 0.5s;
}
.photo:hover{
transform: scale(1.1,1.1);
z-index:4;
transition: 0.5s;
}
.photo {
    /* スライドショーの幅 */
    max-width: 100%; /* スマホで表示が欠けないように */
    margin: 10px auto; /* 中央揃え */
    position: relative;
}
 
.photo img { 
    /* animation: show 20s infinite;  */
    max-width: 100%;
    height: auto;
    opacity: 0;
    position: absolute; /*画像を重ねて表示*/
    left: 0;
    top: 0;
}

.movie{
  position:absolute;
  background-color:white;
  text-align: center;
  top:400px;
  left:50px;
  border-radius: 5px;
  transform: rotate(2deg);
  box-shadow: 4px 4px 8px;
}
.movie p{
  margin: 0px auto;
  font-size: 3em;
  padding: 10px 20px;
  font-family:monospace;
  color: #000;
}
.movie:hover{
  background-color:rgb(170, 241, 250);
  transition:all 0.2s;

}
.movie p:hover {
  color:rgb(248, 133, 113);
  transition:all 0.2s;
  text-shadow:0 3px 3px black;
}
.movie:active{
box-shadow: none;

}

footer{
  height:80px;
  background-color:rgba(1, 20, 187,0.8);
  text-align: right;
  color:#fff;
  padding:10px;
  position:relative;

}
</style>